...
testClass tokenize = do
    describe "Lexer" $ do
        it "should tokenize class" $ do
            tokenize (unlines ["class A {};", "", "class B inherits A {};"]) `shouldBe` [Class, Type "A", LeftSquirly, RightSquirly, SemiColon, Class, Type "B", Inherits, Type "A", LeftSquirly, RightSquirly, SemiColon]
