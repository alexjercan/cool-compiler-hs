...
testClass tokenize = do
    describe "Lexer" $ do
        it "should tokenize class" $ do
            tokens (fromRight [] (tokenize (unlines ["class A {};", "", "class B inherits A {};"]))) `shouldBe` [Class, Type "A", LeftSquirly, RightSquirly, SemiColon, Class, Type "B", Inherits, Type "A", LeftSquirly, RightSquirly, SemiColon, Eof]
...
testAttributeNoInit tokenize = do
    describe "Lexer" $ do
        it "should tokenize attribute no init" $
            do
                tokens (fromRight [] (tokenize (unlines ["class A {", "    x : SELF_TYPE;", "};", "", "class B inherits A {", "    y : Int;", "};"])))
                `shouldBe` [Class, Type "A", LeftSquirly, Ident "x", Colon, Type "SELF_TYPE", SemiColon, RightSquirly, SemiColon, Class, Type "B", Inherits, Type "A", LeftSquirly, Ident "y", Colon, Type "Int", SemiColon, RightSquirly, SemiColon, Eof]

testAttributeInit :: Tokenizer -> Spec
testAttributeInit tokenize = do
    describe "Lexer" $ do
        it "should tokenize attribute init" $ do
            tokens (fromRight [] (tokenize (unlines ["class A {", "    x : SELF_TYPE;", "};", "", "class B inherits A {", "    y : Int <- 0;", "};"])))
                `shouldBe` [Class, Type "A", LeftSquirly, Ident "x", Colon, Type "SELF_TYPE", SemiColon, RightSquirly, SemiColon, Class, Type "B", Inherits, Type "A", LeftSquirly, Ident "y", Colon, Type "Int", Assign, Integer 0, SemiColon, RightSquirly, SemiColon, Eof]

testMethod :: Tokenizer -> Spec
testMethod tokenize = do
    describe "Lexer" $ do
        it "should tokenize method" $ do
            tokens (fromRight [] (tokenize (unlines ["class A {", "    x : SELF_TYPE;", "", "    f() : Object { 0 };", "};", "", "class B inherits A {", "    y : Int <- 0;", "", "    g(x : Int, y : Bool) : Int { 0 };", "};"])))
...
testLiteralId tokenize = do
    describe "Lexer" $ do
        it "should tokenize literal" $ do
            tokens (fromRight [] (tokenize (unlines ["class A {", "    n : Int <- 0;", "    s : String <- \"abc\";", "    b : Bool <- false;", "    c : Bool <- b;", "};"])))
                `shouldBe` [Class, Type "A", LeftSquirly, Ident "n", Colon, Type "Int", Assign, Integer 0, SemiColon, Ident "s", Colon, Type "String", Assign, String "abc", SemiColon, Ident "b", Colon, Type "Bool", Assign, Boolean False, SemiColon, Ident "c", Colon, Type "Bool", Assign, Ident "b", SemiColon, RightSquirly, SemiColon, Eof]

testArithmetic :: Tokenizer -> Spec
testArithmetic tokenize = do
    describe "Lexer" $ do
        it "should tokenize arithmetic" $ do
            tokens (fromRight [] (tokenize (unlines ["class A {", "    n : Int <- 1 + 2 * (3 + 4) / (5 - ~6);", "};"])))
                `shouldBe` [Class, Type "A", LeftSquirly, Ident "n", Colon, Type "Int", Assign, Integer 1, Plus, Integer 2, Asterisk, LeftParen, Integer 3, Plus, Integer 4, RightParen, Slash, LeftParen, Integer 5, Minus, Tilde, Integer 6, RightParen, SemiColon, RightSquirly, SemiColon, Eof]

testRelational :: Tokenizer -> Spec
testRelational tokenize = do
    describe "Lexer" $ do
        it "should tokenize relational" $ do
            tokens (fromRight [] (tokenize (unlines ["class A {", "    n : Int;", "    b : Bool <- not 2 <= n;", "    c : Bool <- 2 < n + 1 = false;", "};"])))
                `shouldBe` [Class, Type "A", LeftSquirly, Ident "n", Colon, Type "Int", SemiColon, Ident "b", Colon, Type "Bool", Assign, Not, Integer 2, LessEqual, Ident "n", SemiColon, Ident "c", Colon, Type "Bool", Assign, Integer 2, LessThan, Ident "n", Plus, Integer 1, Equal, Boolean False, SemiColon, RightSquirly, SemiColon, Eof]

testAssignment :: Tokenizer -> Spec
testAssignment tokenize = do
    describe "Lexer" $ do
        it "should tokenize assignment" $ do
            tokens (fromRight [] (tokenize (unlines ["class A {", "    x : Int;", "", "    f(y : Int) : Int {", "        x <- y", "    };", "", "    f(y : Int, z : Int) : Int {", "        x <- y <- z", "    };", "};"])))
                `shouldBe` [Class, Type "A", LeftSquirly, Ident "x", Colon, Type "Int", SemiColon, Ident "f", LeftParen, Ident "y", Colon, Type "Int", RightParen, Colon, Type "Int", LeftSquirly, Ident "x", Assign, Ident "y", RightSquirly, SemiColon, Ident "f", LeftParen, Ident "y", Colon, Type "Int", Comma, Ident "z", Colon, Type "Int", RightParen, Colon, Type "Int", LeftSquirly, Ident "x", Assign, Ident "y", Assign, Ident "z", RightSquirly, SemiColon, RightSquirly, SemiColon, Eof]

testNewIsVoid :: Tokenizer -> Spec
testNewIsVoid tokenize = do
    describe "Lexer" $ do
        it "should tokenize new isvoid" $ do
            tokens (fromRight [] (tokenize (unlines ["class A {", "    f() : Bool {", "        not isvoid new A", "    };", "};"])))
                `shouldBe` [Class, Type "A", LeftSquirly, Ident "f", LeftParen, RightParen, Colon, Type "Bool", LeftSquirly, Not, IsVoid, New, Type "A", RightSquirly, SemiColon, RightSquirly, SemiColon, Eof]

testDispatch :: Tokenizer -> Spec
testDispatch tokenize = do
    describe "Lexer" $ do
        it "should tokenize dispatch" $ do
            tokens (fromRight [] (tokenize (unlines ["class A {", "    f(x : Int) : Bool {", "        f(x + 1)", "    };", "", "    g(x : Int, y : Int) : Bool {", "        self.g(x + 1, y + 1)", "    };", "", "    h(x : Int) : A {", "        new A@A.h(x + 1).h(x + 2)", "    };", "};"])))
...
testIf tokenize = do
    describe "Lexer" $ do
        it "should tokenize if" $ do
            tokens (fromRight [] (tokenize (unlines ["class A {", "    f(x : Int) : Int {", "        if x <= 5 then x else x + 1 fi", "    };", "};"])))
                `shouldBe` [Class, Type "A", LeftSquirly, Ident "f", LeftParen, Ident "x", Colon, Type "Int", RightParen, Colon, Type "Int", LeftSquirly, If, Ident "x", LessEqual, Integer 5, Then, Ident "x", Else, Ident "x", Plus, Integer 1, Fi, RightSquirly, SemiColon, RightSquirly, SemiColon, Eof]

testWhile :: Tokenizer -> Spec
testWhile tokenize = do
    describe "Lexer" $ do
        it "should tokenize while" $ do
            tokens (fromRight [] (tokenize (unlines ["class A {", "    f(x : Int) : Object {", "        while 0 < x loop", "            x <- x - 1", "        pool", "    };", "};"])))
                `shouldBe` [Class, Type "A", LeftSquirly, Ident "f", LeftParen, Ident "x", Colon, Type "Int", RightParen, Colon, Type "Object", LeftSquirly, While, Integer 0, LessThan, Ident "x", Loop, Ident "x", Assign, Ident "x", Minus, Integer 1, Pool, RightSquirly, SemiColon, RightSquirly, SemiColon, Eof]

testLet :: Tokenizer -> Spec
testLet tokenize = do
    describe "Lexer" $ do
        it "should tokenize let" $ do
            tokens (fromRight [] (tokenize (unlines ["class A {", "    f(x : Int) : Int {", "        let x : Int <- x + 1,", "            y : Int <- x + 1", "        in", "            x", "    };", "};"])))
                `shouldBe` [Class, Type "A", LeftSquirly, Ident "f", LeftParen, Ident "x", Colon, Type "Int", RightParen, Colon, Type "Int", LeftSquirly, Let, Ident "x", Colon, Type "Int", Assign, Ident "x", Plus, Integer 1, Comma, Ident "y", Colon, Type "Int", Assign, Ident "x", Plus, Integer 1, In, Ident "x", RightSquirly, SemiColon, RightSquirly, SemiColon, Eof]

testCase :: Tokenizer -> Spec
testCase tokenize = do
    describe "Lexer" $ do
        it "should tokenize case" $ do
            tokens (fromRight [] (tokenize (unlines ["class A {", "    f(x : Object) : String {", "        case x of", "            s : String => \"String\";", "            i : Int    => \"Int\";", "            o : Object => \"Oops\";", "        esac", "    };", "};"])))
...
testBlock tokenize = do
    describe "Lexer" $ do
        it "should tokenize block" $ do
            tokens (fromRight [] (tokenize (unlines ["class A {", "    f(x : Int) : String {", "        {", "            x <- x + 1;", "            \"Done!\";", "        }", "    };", "};"])))
                `shouldBe` [Class, Type "A", LeftSquirly, Ident "f", LeftParen, Ident "x", Colon, Type "Int", RightParen, Colon, Type "String", LeftSquirly, LeftSquirly, Ident "x", Assign, Ident "x", Plus, Integer 1, SemiColon, String "Done!", SemiColon, RightSquirly, RightSquirly, SemiColon, RightSquirly, SemiColon, Eof]

testStringSpecialChars :: Tokenizer -> Spec
testStringSpecialChars tokenize = do
    describe "Lexer" $ do
        it "should tokenize string with special chars" $ do
            tokens (fromRight [] (tokenize (unlines ["class A {", "    s1 : String <- \"ab\\tcd\";", "    s2 : String <- \"ab\\ncd\";", "    s3 : String <- \"ab\\", "cd\";", "    s4 : String <- \"ab\\\\cd\";", "    s5 : String <- \"ab\\zcd\";", "};"])))
